...
7. **Módulo Operativo (Operativos Oftalmológicos en terreno)** — requerimiento levantado el 2026-09-15 (`OPT_Requerimiento_Modulo_Operativo.md`, aportado por el usuario; escrito originalmente para otro proyecto — multi-tenant, PK `UNIQUEIDENTIFIER`, `IsDeleted` — y adaptado a las convenciones reales de OPT_NET antes de codificar). Objetivo: agrupar las OT de una jornada en terreno bajo un "Operativo", registrar sus gastos, filtrar cobranza/reporte de cristales por Operativo y calcular ganancia/pérdida (vendido y pagado vs. gastos). **Primera etapa (2026-09-15): esquema** — `src/basedatos/009_modulo_operativo.sql` agrega `OPT_EstadoOperativo` (catálogo: PROSPECTO/INGRESADO/COBRANZA/CERRADO/ANULADO), `OPT_Operativo` (`AuditableEntity` + `PublicId`, `EmpresaId`/`SucursalId` obligatorios, `Correlativo` autogenerado vía `SEQ_CorrelativoOperativo`, `MontoTotalVendido`/`MontoTotalPagado`/`MontoTotalGastos` persistidos), `OPT_OperativoOT` (relación pura 1 Operativo→N OT, sin `PublicId` — anidada bajo el Operativo; una OT pertenece a lo sumo un Operativo) y `OPT_GastoOperativo` (`AuditableEntity`; solo Monto + NúmeroDocumento + Observación, sin fecha propia ni categoría). **Segunda etapa, misma sesión (2026-09-15): Domain/Application/API completos**, para que el esquema no quedara desalineado del modelo (el gotcha que sí le pasó a `004` con `Pago`/`Cuota`/`EstadoCuota`) — `OPT_OperativoOT` ganó `MontoVendidoSnapshot`/`MontoPagadoSnapshot` (decimal, ambos DEFAULT 0) respecto al script original de la primera etapa: sin un snapshot por OT, `Operativo.MontoTotalVendido`/`MontoTotalPagado` no se podían mantener como totales corrientes al asociar/quitar una OT sin acoplar el agregado Operativo al agregado Comercial (`OrdenDeTrabajo`). Entidades en `OPT.Domain/Entities/Operativo/` (`Operativo` — raíz del agregado, no referencia `OrdenDeTrabajo` directamente; `OperativoOT`, `GastoOperativo`, `EstadoOperativo`, `EstadosOperativo` con las reglas de flujo: avance secuencial sin retroceso Prospecto→Ingresado→Cobranza→Cerrado, `Anulado` solo alcanzable desde Prospecto/Ingresado — decisión 8.6) y sus `IEntityTypeConfiguration<T>` en `OPT.Infrastructure/Persistence/Configurations/Operativo/`. `IOperativoRepositorio`/`IEstadoOperativoRepositorio` + implementaciones, registrados en `DependencyInjection`. Application: `OPT.Application/Features/Operativos/` (Crear, Actualizar, CambiarEstado, Anular, AsociarOrden, QuitarOrden, RecalcularMontos, RegistrarGasto, EliminarGasto + queries paginada y por Id, más `OperativoDtoFactory`) y `Features/EstadosOperativo/` (catálogo de solo lectura). API: `OperativosController` (`/api/operativos`, siempre por `PublicId`, con `AutorizarRoles` por acción — Anular reservado a supervisión igual que en OT) y `EstadosOperativoController`. La regla "una OT con Operativo debe tener Empresa asociada" se valida en `AsociarOrdenAOperativoCommandHandler`; "no se puede ingresar un gasto si el Operativo está ANULADO" vive en `Operativo.RegistrarGasto`. Ganancia/pérdida (punto abierto 8.3) se devuelve como ambas fórmulas derivadas (`Pagado−Gastos` y `Vendido−Gastos`) en el DTO, sin persistir columna. El filtro de Cobranza por Operativo (sección 6 del requerimiento) se resolvió agregando `operativoPublicId` a `ObtenerOrdenesDeTrabajoQuery`/`IOrdenDeTrabajoRepositorio.BuscarPaginadoAsync` — mismo mecanismo que ya usa el filtro por `empresaPublicId`; el filtro por Operativo en el reporte de cristales sigue sin endpoint porque ese reporte no está implementado en el sistema nuevo. Verificado con `dotnet build OPT.sln` (compila limpio) y `dotnet ef dbcontext info` (el modelo de EF Core valida). **No aplicado aún a `dbOPT_NET`** — pendiente correr `009_modulo_operativo.sql` (ya incluye las columnas de snapshot) y probar los endpoints con una sesión autenticada real.
   ...
   **Tercera etapa, sesión 2026-09-15 (2ª): frontend completo** — `src/frontend/src/app/features/operativos/` (modelos, servicios `Operativos`/`EstadosOperativo`, listado paginado, diálogo de alta/edición y ficha ruteada), siguiendo al pie el patrón ya documentado para el módulo Comercial en `src/frontend/CLAUDE.md` (mismo criterio de "ficha ruteada" que `orden-de-trabajo-ficha`/`cliente-ficha`: agrupa dos historiales de subrecurso a la vez — Órdenes asociadas y Gastos). Alta/edición quedó como diálogo (`OperativoForm`), no página ruteada, porque a diferencia de la OT no incluye una tabla de detalle que no quepa en el panel de `MatDialog`; Empresa/Sucursal son inmutables tras crear y se ocultan/deshabilitan en edición. La ficha reproduce el flujo de estados (Prospecto→Ingresado→Cobranza→Cerrado, una etapa a la vez, sin retroceso — a diferencia de la OT, `EstadosOperativo` del dominio no admite volver atrás) y expone Anular (solo desde Prospecto/Ingresado), Recalcular montos, Asociar/Quitar orden (`AsociarOrdenDialog`, envoltorio de `<app-selector-orden>` ya existente — no se duplicó ese buscador) y Registrar/Eliminar gasto (`GastoOperativoDialog`). El chip de estado (`EstadoOperativoChip`) reutiliza las clases globales `.opt-chip--info/si/alerta` de `styles.scss` en vez de crear tokens de color nuevos — el catálogo de 5 estados no lo justificaba y la regla del branding doc exige auditar contraste WCAG antes de sumar un color. Cambio adicional acotado: `FiltrosOrdenesDeTrabajo`/`OrdenesDeTrabajo.buscar()` y el listado de OT ganaron el filtro de contexto `operativoPublicId` (mismo mecanismo que `empresaPublicId`/`clientePublicId`) para que "Ver en el listado de OT" desde la ficha del Operativo funcione — el backend ya lo soportaba desde la segunda etapa. Registrado en `app.routes.ts` (`/operativos`) y en el grupo "Comercial" del menú de `Shell`. Verificado con `npm run lint` (limpio), `npm run build` (limpio) y `npm test` (65/66 archivos, 105/107 tests — los 2 que fallan son de `orden-de-trabajo-form.spec.ts`, preexistentes a esta sesión y no relacionados). **No verificado en navegador contra `dbOPT_NET` real** — depende de que se aplique `009_modulo_operativo.sql` primero (ver párrafo anterior).
   ...
   Sigue pendiente: migración de OT históricas a Operativos, permisos por rol definitivos (se usaron los grupos de `RolesOPT` ya existentes — `OperacionComercial`/`AnulacionComercial` — sin crear uno nuevo específico para Operativo), y el filtro por Operativo en el reporte de cristales (no tiene endpoint porque ese reporte no está implementado en el sistema nuevo).
