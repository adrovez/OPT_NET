...
| `009_modulo_operativo.sql` | Módulo Operativo (Operativos Oftalmológicos en terreno): +4 tablas (`EstadoOperativo`, `Operativo`, `OperativoOT` —con `MontoVendidoSnapshot`/`MontoPagadoSnapshot`, agregadas al construir el dominio— y `GastoOperativo`) + `SEQ_CorrelativoOperativo`. Esquema y `OPT.Domain`/`IEntityTypeConfiguration<T>`/Application/API completos desde la sesión 2026-09-15 — **aplicado a `dbOPT_NET`** el 2026-09-16 |
| `010_operativo_nombre.sql` | +`OPT_Operativo.Nombre` (`nvarchar(200) NOT NULL`) — título libre de la jornada, a pedido del usuario (sesión 2026-09-16); tabla estaba vacía, sin backfill |
| `011_operativoot_snapshot_montos.sql` | Corrige un desfase real en `dbOPT_NET`: `OPT_OperativoOT` se había creado (primera etapa de `009`) antes de que `MontoVendidoSnapshot`/`MontoPagadoSnapshot` se agregaran al script, y como `009` guarda el `CREATE TABLE` con `IF OBJECT_ID(...) IS NULL`, re-ejecutarlo no las agregó a la tabla ya existente — causaba `Invalid column name` al abrir la ficha de un Operativo. Agrega las 2 columnas con `DEFAULT (0)` (sesión 2026-09-16) — **aplicado a `dbOPT_NET`** |
| `012_operativo_contacto.sql` | +`OPT_Operativo.NombreContacto`/`MailContacto`/`TelefonoContacto` (`nvarchar`, todos NULL) — datos de contacto de la persona de la Empresa a cargo de la jornada, propios de cada Operativo (HU-OP-01/02, sesión 2026-09-22) — **aplicado a `dbOPT_NET`** |
...
Total aplicado en `dbOPT_NET`: **27 tablas** (`006`, `007`, `008` y `010` no agregan tablas). Referencia columna por columna: `src/documentos/Diccionario_Datos_OPT.docx`.
