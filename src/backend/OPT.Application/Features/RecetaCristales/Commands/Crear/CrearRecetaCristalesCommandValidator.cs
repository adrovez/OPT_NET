...
public class CrearRecetaCristalesCommandValidator : AbstractValidator<CrearRecetaCristalesCommand>
{
    // Rango típico de graduación óptica (ver comentario en RecetaCristalesConfiguration).
    private const decimal MinGraduacion = -30m;
    private const decimal MaxGraduacion = 30m;

    public CrearRecetaCristalesCommandValidator()
    {
        ...
        // Cristales Lejos/Cerca" estaba activo (paridad con el legacy, Receta.js). Se relajó a
        // opcional el 2026-09-08 (ADR 0010): el botón Guardar del frontend quedaba bloqueado sin
        // feedback visible y el negocio confirmó que no deberían ser obligatorias.
    }
}
