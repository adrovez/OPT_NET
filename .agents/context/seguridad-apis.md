...
- **`RolesOPT.OperacionComercialConCalidad` ganó Técnico Médico** (sesión 2026-09-22, HU-OP-08 del submenú Recepción del módulo Operativo — `src/documentos/HU/02_HU_Modulo_Operativo.html`): el grupo pasó de [Admin, Supervisor, JefeSucursal, Vendedor, Operador, ControlCalidad] a sumar TecnicoMedico, exactamente el ajuste que este archivo ya anticipaba en el punto "Pendiente relacionado" de abajo. Aplicado a los mismos endpoints que ya usaban ese grupo: `OrdenesDeTrabajoController.ObtenerTodos/ObtenerPorId/CambiarEstado` y `OperativosController.ObtenerTodos/ObtenerPorId` (más el endpoint nuevo `GET /api/operativos/{publicId}/reporte-cristales`, creado ya con este grupo). No se creó un grupo nuevo — se corrigió el existente, con el criterio ya escrito acá.
